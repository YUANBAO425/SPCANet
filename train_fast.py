...
if __name__ == '__main__':
    ...
    ###########加
    # sam_optimizer=SAM(net.parameters(), base_optimizer=torch.optim.Adam, rho=0.01, lr=0.00001, adaptive=False)
    # sam_optimizer=SAM(net.parameters(), base_optimizer=torch.optim.SGD,rho=0.01,lr=0.0001,adaptive=False,weight_decay=.0005, momentum=.9, nesterov=True)
...
#         if epoch_index % opt.eval_per_epoch == 0 and epoch_index > 1:
            ...
            save_now = validate_MAE < base_mae
            base_mae = min(validate_MAE,base_mae);
            ...
            if opt.extra_loss:
                ...
                for (x_offset) in zip(x_offset_list):
                    ...
                    if opt.extra_loss:
                        ...
                        A_y = x_offset[0][:, 0, :, :]
                        A_x = x_offset[0][:, 1, :, :]
                        B_y, B_x = x_offset[0][:, 2, :, :], x_offset[0][:, 3, :, :]
                        C_y, C_x = x_offset[0][:, 4, :, :], x_offset[0][:, 5, :, :]
                        D_y, D_x = x_offset[0][:, 6, :, :], x_offset[0][:, 7, :, :]
                        E_y, E_x = x_offset[0][:, 8, :, :], x_offset[0][:, 9, :, :]
                        F_y, F_x = x_offset[0][:, 10, :, :], x_offset[0][:, 11, :, :]
                        G_y, G_x = x_offset[0][:, 12, :, :], x_offset[0][:, 13, :, :]
                        H_y, H_x = x_offset[0][:, 14, :, :], x_offset[0][:, 15, :, :]
                        I_y, I_x = x_offset[0][:, 16, :, :], x_offset[0][:, 17, :, :]
